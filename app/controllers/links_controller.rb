...
class LinksController < ApplicationController
  ...
  def create
    ...
    if @link.save
      unless current_user
        ...
        hashes << {
          hash: %r{.*/([0-9A-Z]+)\b}.match(@link.custom_url)[1],
      ...
      unless old_hashes.empty?
        old_hashes.each do |hash|
  ...
  def link_params
    ...
    regexp = /([\s\S]*)http/
    _[:long_url] = 'http://' << _[:long_url] if _[:long_url] != '' && regexp.match(_[:long_url]).nil?
